...
def to_csv_file(FILE_NAME: str, list_headers: list, rows: list[list], delimiter = ',', new_line = '\n'):
    with open(FILE_NAME, 'w') as f:
        f.write(delimiter.join(str(_) for _ in list_headers) + new_line)  # запись в файл списка заголовков
        for list in rows:
            f.write(delimiter.join(str(_) for _ in list) + new_line)  # запись в файл списка с данными
